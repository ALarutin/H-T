...
import (
	"data_base/controllers/forum"
	"data_base/controllers/post"
	"data_base/controllers/service"
	"data_base/controllers/thread"
	"data_base/controllers/user"
...
func GetRouter() (router *mux.Router) {
	...
	logger.Info.Printf("Initialization router\n")
	...
	for _, router := range forumRouters {
		forumSubRouter.
			Methods(router.Method).
			Name(router.Name).
			Path(router.Path).
			HandlerFunc(router.Handler)
		logger.Info.Printf("Initialization router\nname:    %s\ninfo:    %s\nmetod:   %s\n",
			router.Name, router.Info, router.Method)
	}
	...
	for _, router := range postRouters {
		postSubRouter.
			Methods(router.Method).
			Name(router.Name).
			Path(router.Path).
	...
	serviceRouters := []bootstrap.Router{
		{
	...
	for _, router := range serviceRouters {
		serviceSubRouter.
			Methods(router.Method).
			Name(router.Name).
			Path(router.Path).
	...
	threadRouters := []bootstrap.Router{
		{
	...
	for _, router := range threadRouters {
		threadSubRouter.
			Methods(router.Method).
			Name(router.Name).
			Path(router.Path).
